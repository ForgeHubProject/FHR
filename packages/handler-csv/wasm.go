...
// WebAssembly entry point for the csv handler. Built with `GOOS=js GOARCH=wasm`,
// it exposes the same diff logic as the native subprocess binary so a diff can
// be computed client-side (SPEC-RENDERING.md §4, Tier B) with no producer/
// consumer skew.
package main
...
func main() {
	...
	js.Global().Set("__forgeHandlerCsv", api)
	select {}
}
